...
typedef struct _hslot { // 각 중복 파일 세트의 정보 HNode slot
	unsigned char hash[41]; // md5 or sha1
	off_t size; // 파일 크기
...
void FInsert(List *plist, char *fpath, time_t atime, time_t mtime); // 각 파일 세트에 첫 파일 저장
void NInsert(List *plist, char *fpath, time_t atime, time_t mtime); // 각 파일 세트에 두번째 이후의 파일 저장
